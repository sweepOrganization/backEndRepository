...
@Tag(name = "즐겨찾기 위치", description = "즐겨찾기 위치 관련 API")
@RestController
...
public class FavoriteLocationController {
    ...
    @Operation(
            ...
            description = """
                    새로운 즐겨찾기 위치를 등록합니다.
                    - name, address, x, y, locationType
                    - member_id 인증 토큰에서 자동추출
                    - 동일 회원의 name 중복 불가능
                    - HOME·WORK 유형은 회원당 각 1개만 등록 가능합니다.
    ...
    @Operation(
            summary = "내 즐겨찾기 위치 목록 조회",
            description = """
                    로그인한 사용자의 즐겨찾기 위치 목록을 조회합니다.
                    - HOME → WORK → CUSTOM 순으로 상단 고정되며, 같은 유형 내에서는 createdAt DESC 정렬하도록 유도했습니다.
                    """
    )
    @ApiResponses(value = {
